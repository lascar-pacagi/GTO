...
struct CFR_Plus {
    ...
    double cfr(Game& g, int player, double pi_player, double pi_other, int iter) {
        if (g.game_over()) {
        ...
        if (g.is_chance_node()) {
            ...
            g.play(action);
            double v = cfr(g, player, pi_player, pi_other, iter);
            g.undo(action);
            return v;
        }
        ...
        for (int i = 0; i < n.nb_actions; i++) {
            ...
            if (current_player == player) {
                utils[i] = cfr(g, player, s[i] * pi_player, pi_other, iter);
            } else {
                utils[i] = cfr(g, player, pi_player, s[i] * pi_other, iter);
            }
            g.undo(a);
            u += s[i] * utils[i];
        }
    ...
    void solve(size_t nb_iterations) {
        ...
        for (size_t i = 1; i <= nb_iterations; i++) {
            for (auto player : { PLAYER1, PLAYER2 }) {
                g.reset();
                game_value[player] += cfr(g, player, 1, 1, i);
            }            
        }
        for (auto player : { PLAYER1, PLAYER2 }) {
            Node& n = nodes[player];
